...
void UART0_Init(void) /* UART0 configuration: 1 start, 8 bits data, No Parity, 1 stop bit and 9600BPS */
{
    ...
    while (!(SYSCTL->PRUART & 0x01)); /* Wait until UART0 clock is activated and it is ready for access*/
...
{
    while (!(UART0->FR & UART_FR_TXFE_MASK));/* Wait until the transmit FIFO is empty */
		UART0->DR = data;
}

uint8_t UART0_ReceiveByte(void)
{
		while (UART0->FR & UART_FR_RXFE_MASK); /* Wait until the receive FIFO is not empty */
