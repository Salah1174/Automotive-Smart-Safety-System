...
void NVIC_EnableException(NVIC_ExceptionType Exception_Num)
{
        Enable_Exceptions();
        Enable_Faults();
        if (Exception_Num > 15 || Exception_Num <= 2)
        {                // cant modify NMIn Reset, or stack pointer
                blink(); // Alert user about error in entry
                return;
        }
        ...
        case 2:
                return;
        case 3:
                NVIC_SYSTEM_SYSHNDCTRL |= MEM_FAULT_ENABLE_MASK; // memory
                break;
        case 4:
                NVIC_SYSTEM_SYSHNDCTRL |= BUS_FAULT_ENABLE_MASK; // bus
                break;
        case 5:
                NVIC_SYSTEM_SYSHNDCTRL |= USAGE_FAULT_ENABLE_MASK; // usage
                break;
...
void NVIC_DisableException(NVIC_ExceptionType Exception_Num)
{
        ...
        Disable_Faults();
        if (Exception_Num > 15 || Exception_Num <= 2)
        {                // cant modify NMI, Reset, or stack pointer
                blink(); // Alert user about error in entry
                return;
        }
        switch (Exception_Num)
        ...
        case 2:
                return;
        case 3:
                NVIC_SYSTEM_SYSHNDCTRL &= ~(1 << 16); // memory
                break;
        case 4:
                NVIC_SYSTEM_SYSHNDCTRL &= ~(1 << 17); // bus
                break;
        case 5:
                NVIC_SYSTEM_SYSHNDCTRL &= ~(1 << 18); // usage
                break;
        case 6: // NVIC_SYSTEM_SYSHNDCTRL &= ~(1<<15); //SVC
                // break;
                return;
